...
The worktree is the only working tree checked proactively. There's deliberately **no** cleanliness gate on the default-branch checkout — the fast-forward in step 3 polices itself, and pre-checking it is both redundant and too strict (it would trip over harmless untracked files like a sibling skill awaiting this merge's gitignore change).
...
2. **If the session lives in `$WT`, detach the harness now — before the fast-forward, not at teardown.** A worktree-isolated Claude Code session refuses *any* `git -C` that targets a path outside its own worktree ("this command redirects git to the shared checkout via `-C`"), and step 3 targets `$DEFAULT_WT`/`$SAFE` — so while pinned, the fast-forward cannot run at all. Detach with the **`ExitWorktree` tool, `action: "keep"`** (see *Removing the worktree + branch* step 3 for why `keep` and when to skip it); it leaves the worktree and branch fully intact on disk, so nothing about the merge changes — only the session's cwd. Do it **after** the rebase, which runs inside `$WT` and is fine while pinned. Once detached, `git -C "$WT" …` works too, so the step-3 retry loop still rebases normally.
3. Fast-forward the default branch to `$BR`. After the rebase `$BR` is a strict descendant of `$DEFAULT`, so this can't create a merge commit:
   ...
   ```
   - **Non-zero → beaten.** `$DEFAULT` now points onto a commit that isn't in `$BR`'s history. Go back to **step 1** and redo the rebase: `git -C "$WT" rebase "$DEFAULT"` picks up the now-current tip (re-resolve any fresh conflicts against the just-landed work, as in step 1), then re-attempt this fast-forward. Loop until it lands. Step 2 is already done and isn't repeated — detaching is idempotent, and a second `ExitWorktree` would be a no-op anyway.
...
4. **Tear down** the worktree + branch — run *Removing the worktree + branch* below. Its detach step is already satisfied by step 2, so it goes straight to the removal. After the fast-forward the branch is merged and the worktree is normally clean, so it removes without prompting; if conflict resolution left untracked files behind, the dirty check there catches them and confirms first.
5. **Report:** `$BR` fast-forwarded onto `$DEFAULT` (N commits), worktree + branch removed. `$DEFAULT` is now **ahead of origin — do not push** (the user pushes when ready).
...
3. **If the session lives in `$WT`, detach the harness first.** (`/wt merge` has already done this at its own step 2, because the fast-forward can't run while pinned; re-running here is an `ExitWorktree` no-op, so this step is really `/wt delete`'s.) When the current Claude Code session is working *inside* `$WT` — an `EnterWorktree`-made worktree; the usual no-argument `/wt merge`/`/wt delete`, or an argument naming your own worktree — the harness pins the session's cwd inside `$WT` and keeps resetting it back there, so the raw `git worktree remove` in step 4 would strand it. Being pinned also blocks the `git -C "$SAFE"` removals in step 4 outright, the same way it blocks `/wt merge`'s fast-forward. Detach first with the **`ExitWorktree` tool, `action: "keep"`**: `keep` leaves the worktree *and* branch on disk untouched (step 4 does the actual removal) and restores the session's cwd to outside `$WT`. Because it's `keep`, not `remove`, it trips **none** of `ExitWorktree`'s guards — no spurious "N commits will be discarded" refusal for work `/wt merge` already fast-forwarded onto `$DEFAULT`.
...
- Operating on the worktree the **current Claude Code session lives in** detaches the harness with `ExitWorktree action: "keep"` first, because a worktree-isolated session both pins its cwd inside `$WT` and refuses any `git -C` aimed outside it. `/wt merge` therefore detaches right after the rebase (step 2) rather than at teardown — its fast-forward targets the default branch's checkout and is blocked while pinned; `/wt delete` detaches in the teardown, before the git removal. `keep` carries no discard guard, so a `/wt merge` whose commits are already fast-forwarded onto the local default branch tears down without a false "N commits will be discarded" prompt.
