...
**Where it lives — `/tmp`, never the repo.** The plan doc is an **ephemeral working artifact** (live status, decisions, the twists and turns of execution), not project documentation. Keep it at a stable, deterministic path under `/tmp` — e.g. `/tmp/<effort>-plan.md` — so a resumed orchestrator can find it again, and **never inside the version-controlled tree.** Committing it pollutes history with scratch churn that the effort's distillation would only have to strip back out, and clutters every feature commit's diff. If you find a prior effort left its plan inside the repo, move it to `/tmp` rather than perpetuating it.

**On invocation, check for an existing doc first.** If a plan doc for this effort already exists at its `/tmp` path, read it in full and **resume** from the recorded status — pick up at the first not-done stage; don't re-plan what's already recorded. Only when there's no existing doc do you establish one.

**Establishing it.** If the human pointed you at an implementation plan, adopt it (copying it to the `/tmp` working path if it lived elsewhere). If they pointed you at a design doc (the *what/why*) with no execution plan, produce the plan first — the staged *how/sequence* — and get the human's sign-off before executing.

**Design doc vs. plan doc.** When the effort has a separate design doc, keep the two distinct — including *where they live*: the **design doc** is durable documentation and stays in the repo (authoritative for *intent* — what/why), while the **plan doc** is ephemeral execution state (status/sequencing) and stays in `/tmp`. When execution invalidates a design decision, surface it to the human as a fork — don't silently rewrite the design doc. When the effort completes, fold any durable whys back into the design doc (or wherever the project keeps lasting documentation); the plan doc has then served its purpose and is simply discarded (it was never committed).
